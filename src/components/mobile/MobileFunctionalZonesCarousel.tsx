        ...
        <CarouselContent className="px-4">
          {zonesData.map((zone) => (
            <CarouselItem key={zone.id} className="basis-[220px] pl-2 pr-2" onTouchStart={handleItemTouch}>
              <div className="h-full transition-all duration-300 ease-in-out hover:scale-[1.03] active:scale-[0.98]">
                {/* Image card */}
                ...
                {/* Info block */}
                <div className="pt-3 px-1 pb-1 text-left">
                  {/* Zone name */}
                  <h3 className={cn(
                  ...
                  {/* Enterprise count */}
                  <div className={cn("flex items-center text-xs mb-1", secondaryTextClass)}>
                    <Building2 className="w-4 h-4 mr-1.5" />
                    <span>{zone.enterprises} {t('functionalZones.enterprises')}</span>
                  </div>
                  
                  {/* Occupancy rate with visual bar */}
                  <div className={cn("text-xs mb-1", secondaryTextClass)}>
